...
server <- function(input, output) {
  ...
  output$phenophasePlot <- renderPlot({
    ggplot(phenoData, aes(.phenoData[[input$radio]],reorder(species, .phenoData[[input$radio]], FUN = median, na.rm = TRUE))) +
      geom_violin(aes(fill = reorder(species, .phenoData[[input$radio]], FUN = median, na.rm = TRUE)), show.legend = FALSE, na.rm = TRUE) +
